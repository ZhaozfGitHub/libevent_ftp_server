...
{
	// 水平触发 只要没有接受完成，会再次进来，就不用循环读fd了
	char buf[2] = { 0 };
#ifdef _WIN32
	int re = recv(fd, buf, 1, 0);	// 表面上像socket的recv，实际上像管道的read
#else
...
void XThread::AddTask(XTask* t)
{
	...
	t->base = this->base;
	tasks_mutex.lock();			// 互斥锁，像linux的pthread_mutex
	tasks.push_back(t);
	tasks_mutex.unlock();
}
// 线程激活
...
{
#ifdef _WIN32
	int re = send(this->notify_send_fd, "c", 1, 0);  // linux里的read，不是socket的send
#else
...
bool XThread::Setup()
{
...
#ifdef _WIN32
	// 创建一个socketpair 可以互相通信 fds[0] 读 fds[1]写，跟linux管道差不多，读写也像管道
	...
	// 添加管道监听事件，用于激活线程执行任务
	event* ev = event_new(base, fds[0], EV_READ | EV_PERSIST, NotifyCB, this); // 必须是持续事件
	event_add(ev, 0);
